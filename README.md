...
- **收支总览**：首页实时显示当前激活账单的收入 / 支出 / 盈余
- **自定义账单周期**：账单起止日期完全自由（例如 7/15 ~ 8/15），系统按天生成录入表单，不强制按自然月
- **账单管理**：账单列表按新旧排序，右键可将任意账单设为当前激活账单，也可删除账单（级联删除其下全部记录）
- **分类管理**：内置常用分类，支持自由增删改分类、自定义颜色标签；如需备注细节可直接写在每笔记录的备注里
- **消费报告**：按账单生成分类占比饼图 + 明细表格，支持导出 **PDF**（含图表，适合打印分享）和 **Excel**（汇总 + 流水明细两个 sheet）
- **内置计算器**：任意页面右下角悬浮按钮唤起，支持鼠标点击和键盘输入两种方式，四则运算，方便录入前心算
